...
//	@GetMapping("/member/login")// GET 방식의 /member/login 요청을 연결
	...
	// -> Spring Container에서 Arguement Resolver(매개변수 해결사) 제공해서 
	// 	유연하게 처리함 
	...
	public String login(/*@ModelAttribute*/ Member inputMember,
		...
		// - request scope가 기본값.
		// + @SessionAttributes 어노테이션과 함께 작성시 
		...
		if(loginMember!=null) {
			...
			// addAttirbute("K",V)== req.setAttribute("K",V)
			...
			// 로그인 실패 시 "아이띠 또는 비밀번호가일치하지 않숩니다"세션에 추가 
//			model.addAttribute("message", "아이디 또는 비밀번호가 일치하지 않습니다");
			// -> 메인 페이지 주소에 message 값 노출 
			// -> RedirectAttributes로 변환 
			
			ra.addFlashAttribute("message", "아이디 또는 비밀번호가일치하지 않습니다");
